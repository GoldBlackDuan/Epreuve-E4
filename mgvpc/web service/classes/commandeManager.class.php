...
class commandeManager {
     ...
     * Sélectionne une commande dans la base.
     * 
     * Méthode de SELECT qui renvoie l'ensmeble des produits dans la commande dont le numéro est spécifié en paramètre.
     * 
     * @param int Numéro de la commande 
     * @return produit|boolean Renvoie les produits de la commande ou FALSE si elle n'a pas été trouvée
     */
    public function getProduitsParCommande($num)
    {
        $query = "select * from `produit`, `comporter` WHERE produit.idProd = comporter.idProd AND comporter.numCde=?;";
        ...
        while ($row =$traitement->fetch())
        {
            ...
            $produit = new produit($row['libProd'],$row['descProd'],$row['prixProd']);
